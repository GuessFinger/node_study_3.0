...
node的模块划分
1. 核心模块 内置模块  fs  path  http 
2. 文件模块  .js  .json .node
3. 第三方模块   mime


require() 是同步的 这个需要注意一下

- require加载括号里面的两种写法

- 情况1 后面的参数是路径
require('./index.js') 会根据./这个路径 转换成绝对路径 然后去找这个文件 如果找不到就加载失败
如果不带后缀的话  先根据路径找  index.js  index.json index.node  
最后index文件夹 -> package.json -> main(入口文件 app.js -> index.js/index.json/index.node) ->  加载失败  
require('./index')    

- 情况2 后面的参数不是路径 直接是模块名称
require('fs')
先查看是否加载的是核心模块 如果是的话 则直接加载该核心模块   如果不是的话
就会在当前的js文件的目录下去查找是否有一个 node_modules (如果没有的话 就会去当前文件的父目录去寻找node_modules)  
    然后去寻找第三方模块
    
require加载模块的注意点
1. 所有的模块第一次加载完毕后 以后都有缓存  二次加载直接读取缓存 避免二次开销(因为有缓存 所以模块中的代码只有第一次加载的时候执行一次)
2. 核心模块子node.js源码编译的时候 已经成为二进制执行文件 所以加载非常快
3. 自定义模块命名需要注意  不要和核心模块重名
4. 引入第三方模块的时候 后缀一定要添加上  要不然这个寻找非常的慢


梳理
- 使用require加载模块的时候 会把需要加载的模块中的所有代码都执行一遍
- require加载模块后 会缓存起来 模块中代码不会再次执行了


ECMA 规范的是js的基本使用
CommonJs 规范的是更大的一些场景  模块化的开发


模块化开发 感觉主要是为了维护 以及开发方法 
- 启动服务模块
- 公共方法模块
- 配置文件模块
- 业务处理模块
